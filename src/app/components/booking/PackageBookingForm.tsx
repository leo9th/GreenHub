...
export default function PackageBookingForm() {
  ...
  const submit = useCallback(async () => {
    if (!authUser?.id) {
      ...
      navigate(`/login?next=${encodeURIComponent("/home?service=send")}`);
      return;
    }
    if (!packageDesc.trim()) {
      toast.error("Describe what you’re sending.");
      return;
    }
    ...
    if (pickupLat == null || pickupLng == null || dropoffLat == null || dropoffLng == null) {
      toast.error("Pick addresses from suggestions or use GPS for pickup.");
      return;
    }
    ...
    setSubmitting(true);
    try {
      const riderNote = JSON.stringify({ kind: "package", description: packageDesc.trim() });
      ...
      toast.success("Package request sent.", { description: "Matching you with a nearby rider." });
  ...
  return (
    <section className="mt-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
      <h2 className="text-lg font-semibold text-gray-900 dark:text-foreground">Send a Package</h2>
      <p className="mt-1 text-sm text-gray-600 dark:text-muted-foreground">Pickup, drop-off, and a short description of the item.</p>

      <div className="mt-4 space-y-3">
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-800 dark:text-foreground">What are you sending?</label>
          <textarea
            value={packageDesc}
            onChange={(e) => setPackageDesc(e.target.value)}
            rows={3}
            placeholder="Size, weight, fragile, etc."
      ...
      >
        ...
        {submitting ? "Submitting…" : "Request courier"}
      </button>
    </section>
  );
}
