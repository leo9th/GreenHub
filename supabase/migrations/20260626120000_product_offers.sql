-- Price offers / negotiation per listing.
-- `product_id` type must match `public.products.id` (this project uses UUID in hosted DB).
-- If a failed attempt created `product_offers` with the wrong FK, run once:
...
-- At most one active pending offer per buyer per listing
...
-- Buyers create offers only on others' listings
...
-- Sellers see offers on their products
...
-- Sellers: respond to offers; buyers: edit own pending offer (price / message)
...
-- Buyers may delete own pending offers (withdraw)
