-- Idempotent script: product_offers + RLS + Realtime (same as migration).
-- Mirrors: supabase/migrations/20260626120000_product_offers.sql
-- `products.id` must be UUID (matches hosted Supabase). If an old attempt failed, run first:
